...
export default function ProfilePage() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#170e2c]">
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="relative h-60 w-full bg-[#170e2c] overflow-hidden">
      ...
      <main className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 -mt-32 pb-20">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">

          {/* Left Sidebar: Sticky Profile Card */}
          {/* Left Sidebar: Profile Card (Sticky Removed) */}
          <aside className="lg:col-span-4">
            <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl shadow-[#170e2c]/10 border border-slate-200">
              <div className="flex flex-col items-center px-6 pt-12 pb-8 text-center">
                <div className="relative mb-6 h-36 w-36 group">
                ...
                <h1 className="text-2xl font-black text-[#170e2c]">{profile?.name || "Your Name"}</h1>
                <p className="text-[#5f5aa7] font-bold text-sm tracking-wide uppercase mt-1">
              ...
              <div className="px-8 pb-10 space-y-5 border-t border-slate-50 pt-8">
                <div className="flex items-center gap-4 text-sm text-[#3e3875] font-medium p-3 rounded-2xl bg-slate-50 border border-slate-100">
          ...
          {/* Right Content: Unified Single Container */}
          <div className="lg:col-span-8">
            <div className="bg-white rounded-[2.5rem] shadow-xl shadow-[#170e2c]/5 border border-slate-100 p-8 md:p-12 space-y-12">

              {/* Summary Section */}
              <section>
                <div className="flex items-center gap-3 mb-6">
                  <div className="h-10 w-1 bg-[#5f5aa7] rounded-full" />
                  <h2 className="text-xl font-black text-[#170e2c]">Professional Summary</h2>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {Array.isArray(applicant?.skills) && applicant.skills.length > 0 ? (
                    applicant.skills.map((skill, i) => (
                      <span key={i} className="px-5 py-2.5 rounded-2xl bg-[#f1f0fb] text-[#3e3875] text-sm font-bold border border-[#3e3875]/5 hover:border-[#5f5aa7]/30 transition-all cursor-default">
                        {skill}
                      </span>
                    ))
                  ) : (
                    <p className="text-slate-400">Skills list is empty.</p>
                  )}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-1 bg-[#5f5aa7] rounded-full" />
                    <h2 className="text-xl font-black text-[#170e2c]">Work History</h2>
                  </div>
                    ...
                    applicant.experience.map((exp, i) => (
                      <div key={i} className="group relative pl-10">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start mb-2">
                          <div>
                            <h3 className="text-lg font-bold text-[#170e2c] group-hover:text-[#5f5aa7] transition-colors">{exp.role}</h3>
                            <p className="text-[#7270b1] font-semibold">{exp.company}</p>
                          </div>
                          <span className="mt-1 sm:mt-0 text-[11px] font-black uppercase tracking-widest text-white bg-[#3e3875] px-3 py-1.5 rounded-lg shadow-sm">
                            {exp.year}
                          </span>
                        </div>
                        <p className="text-slate-600 leading-relaxed text-sm mt-3">{exp.description}</p>
                      </div>
                    ))
                  ) : (
                    <p className="text-center text-slate-400 py-4">No experience added.</p>
                  )}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center justify-between mb-10">
                  ...
                  <GraduationCap className="text-slate-200" size={32} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {Array.isArray(applicant?.education) && applicant.education.length > 0 ? (
                    applicant.education.map((edu, i) => (
                      <div key={i} className="p-6 rounded-[2rem] bg-slate-50 border border-slate-100 hover:bg-white hover:shadow-xl hover:border-[#5f5aa7]/20 transition-all duration-300">
                        <div className="flex items-center gap-3 mb-4">
                          <div className="p-2 bg-white rounded-xl shadow-sm">
                            <Award className="text-[#5f5aa7]" size={20} />
                          </div>
                          <span className="text-xs font-black text-[#7270b1] uppercase tracking-tighter">{edu.year}</span>
                        </div>
                        <h3 className="font-bold text-[#170e2c] text-base leading-tight">{edu.school}</h3>
                        <p className="text-[#3e3875] font-medium text-sm mt-1">{edu.degree}</p>
                      </div>
                    ))
                  ) : (
                    <p className="text-slate-400">No education history.</p>
                  )}
                </div>
              </section>
