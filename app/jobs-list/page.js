      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <header className="bg-white border-b border-slate-200/60">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8">
            ...
            {/* SEARCH & FILTERS BAR */}
            <div className="bg-[#e6e2ed] p-1.5 sm:p-2 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl shadow-[#3e3875]/20">
              <div className="flex flex-col lg:flex-row gap-1.5 sm:gap-2">
                <div className="flex-[3] flex flex-col md:flex-row items-center gap-1.5 sm:gap-2">
                  {/* Title Search */}
                  <div className="w-full flex items-center bg-white rounded-xl sm:rounded-2xl px-3 sm:px-4 border border-white/20 focus-within:ring-2 focus-within:ring-[#3e3875]/10 transition-all">
                    <Search size={16} className="text-[#7270b1] shrink-0" />
                    <input
                      value={q} onChange={(e) => setQ(e.target.value)}
                      placeholder="Role or Keywords..."
                      className="w-full bg-transparent py-2.5 sm:py-3 px-2 outline-none text-xs sm:text-sm font-bold placeholder:text-slate-500"
                    />
                  </div>
                  {/* Location Search */}
                  <div className="w-full flex items-center bg-white rounded-xl sm:rounded-2xl px-3 sm:px-4 border border-white/20 focus-within:ring-2 focus-within:ring-[#3e3875]/10 transition-all">
                    <MapPin size={16} className="text-[#7270b1] shrink-0" />
                    <input
                ...
                <div className="flex-[2] flex flex-row items-center gap-1.5 sm:gap-2">
                  <select
                    value={type} onChange={(e) => setType(e.target.value)}
                    className="flex-1 bg-white rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2.5 sm:py-4 text-[11px] sm:text-[13px] font-black outline-none cursor-pointer hover:bg-slate-50 transition-all appearance-none"
                  >
                  ...
                  <button
                    onClick={fetchJobs}
                    className="flex-1 lg:flex-none bg-[#3e3875] hover:bg-[#170e2c] text-white px-4 lg:px-8 py-2.5 lg:py-4 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs uppercase tracking-widest transition-all shadow-lg active:scale-95"
                  >
                    Search
                  </button>
                </div>
